...
class HistoryFirebaseController {
  ...
  Future<bool> checkEpisodeExists( String documentId, String childDocumentID) async {
    try {
      // Truy cập vào tài liệu dựa trên ID trong Firestore
      DocumentSnapshot documentSnapshot = await db
          .collection('users')
  ...
  Future addHistoryMovie(String name, String slug, String thumb_url, String poster_url) async {
  ...
  Future addEpisode(String movieName, String slug, String thumb_url, String poster_url, String movie_url, String episodeName,  int index) async {
  ...
  Future<void> updateWatchingTime( String slug, String episodeName, int currentWatchingTime) async {
    final DocumentReference documentReference = db.collection('users')
        .doc(getCurrentEmail())
        .collection('history')
        .doc(slug)
