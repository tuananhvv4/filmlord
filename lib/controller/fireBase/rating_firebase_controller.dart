...
class RatingFirebaseController {
  ...
  Stream<QuerySnapshot> ratingQuery(String slug){
    final querySnapshot = db.collection('movies')
        .doc(slug)
        .collection('rating')
  ...
  // Thêm thông tin phim
  Future addMovie(String name, String slug, String thumb_url, String poster_url) async {

    bool existDocument = await checkDocumentExists(slug);
    if(!existDocument) {
      try {
        final DocumentReference documentReference = db.collection('movies')
            .doc(slug);
  ...
  // Thêm điểm đánh giá
  Future addRating(int ratePoint, String name, String slug, String thumbUrl,String posterUrl,BuildContext context) async {
    ...
    try {
      ...
      // ignore: use_build_context_synchronously
      QuickAlert.show(context: context, type: QuickAlertType.error, confirmBtnText: 'OK', title: 'Có lỗi xảy ra, vui lòng thử lại!');
    }
  }
  ...
  // Cập nhật đánh giá
  Future updateRating(int newRatePoint, String docID, String slug,BuildContext context) async {
  ...
  // Xóa đánh giá
  Future<void> removeRating(String slug, String docID, BuildContext context) async {
    final DocumentReference documentReference = db.collection('movies')
        .doc(slug)
        .collection('rating')
        .doc(docID);
