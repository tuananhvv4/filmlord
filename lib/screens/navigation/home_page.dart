...
class _HomePageState extends State<HomePage> {
  ...
  late Future<void> _futureNewMovies,
      _futureSingleMovies,
      _futureSeriesMovies,
      _futureCartoonMovies,
      _futureTvShowsMovies;

  // List Data
  List<Data> newMoviesList = [];
  List<Data> singleMoviesList = [];
  List<Data> seriesMoviesList = [];
  List<Data> cartoonMoviesList = [];
  List<Data> tvShowsList = [];

  // Lưu danh sách dữ liệu vào bộ nhớ
  Future<void> saveData(String category, List<Data> data) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String> personList = data.map((person) => jsonEncode(person.toJson())).toList();
    prefs.setStringList(category, personList);
    prefs.setInt('$category-currentPage', 1); // lưu lại trang đã tải
  }
  ...
  Future fetchNewMoviesData() async {
    if (stateController.isLoadData.value == false) {
      ...
      saveData('phim-moi', newMoviesList);
      isLoaded = true;
    }

  }
  Future fetchSingleMovieData() async {
    singleMoviesList =
        await compute(apiServices.fetchMoviesData, 'phim-le');
    saveData('phim-le', singleMoviesList);

  }

  Future fetchSeriesMovieData() async {
    seriesMoviesList =
        await compute(apiServices.fetchMoviesData, 'phim-bo');
    saveData('phim-bo', seriesMoviesList);

  }

  Future fetchCartoonMovieData() async {
    cartoonMoviesList = await compute(
        apiServices.fetchMoviesData, 'hoat-hinh');
    saveData('hoat-hinh', cartoonMoviesList);

  }

  Future fetchTvShowsMovieData() async {
    tvShowsList =
        await compute(apiServices.fetchMoviesData, 'tv-shows');
    saveData('tv-shows', tvShowsList);
  }

  // Future<void> setCacheData(String category, String value) async {
  //   final SharedPreferences prefs = await SharedPreferences.getInstance();
  //   await prefs.setString(category, value); // Xóa tất cả dữ liệu đã lưu
  // }
  ...
  Future playNow(String slug) async {
    ...
    var response =
        await http.get(Uri.parse('https://phimapi.com/phim/$slug'));
    ...
    if(stateController.loginState.value){
      await historyController.addEpisode(movieDetail.name.toString(), movieDetail.slug.toString(), movieDetail.thumbUrl.toString(), movieDetail.posterUrl.toString(), videoUrlList[0], episodeNameList[0], 0);
    }
    Get.to(LandscapePlayer(videoUrlList: videoUrlList, movieDetail: movieDetail, movieEpisodeNameList: episodeNameList, currentIndex: 0,));
  }
  ...
  void initState() {
    ...
    _futureSingleMovies = fetchSingleMovieData();
    _futureSeriesMovies = fetchSeriesMovieData();
    _futureCartoonMovies = fetchCartoonMovieData();
    _futureTvShowsMovies = fetchTvShowsMovieData();
  }
        ...
        child: ListView(
          children: [
                                ...
                                children: [
                                      ...
                                      child: Container(
                                        ...
                                        child: ElevatedButton(
                                          onPressed: playButtonState
                                              ? () {
                                            playNow(newMoviesList[stateController
                                                .currentCarouselIndex.value]
                                                .slug
                                                .toString());
                                          }
                                  ...
                                  // đã đăng nhập
                                    ...
                                    stream: favoriteController.isFavoriteQuery(
                                        newMoviesList[stateController.currentCarouselIndex.value].slug.toString()),
                                    builder: (context, snapshot) {
                                            ...
                                            child: Container(
                                              ...
                                              child: ElevatedButton(
                                                onPressed: () {
                                                  if (hasMovie) {
                                                    ...
                                                    favoriteController.addFavoriteMovie(
                                                        ...
                                                        newMoviesList[stateController
                                                            .currentCarouselIndex
                                                            .value].thumbUrl.toString(),
                                                        newMoviesList[stateController
                                                            .currentCarouselIndex
                                                            .value].posterUrl.toString()
                                                    );
                                                  }
                                                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Thao tác thành công!'),duration: Duration(seconds: 1),));
                                                },
                                                style: ElevatedButton.styleFrom(
                                                ...
                                                child:  Row(
                                                  ...
                                                  children: [
                                                    Icon(
                                                      hasMovie? Icons.check : Icons.add,
                                                      color: Colors.white,
                                                    ),
                                                    const Text(
                                                      'Danh sách',
                                  ...
                                  // chưa đăng nhập
                                      ...
                                      child: Container(
                                        ...
                                        child: ElevatedButton(
                                          onPressed: () {
                                            Navigator.push(context, MaterialPageRoute(builder: (context) => const Common(),));
                                          },
                                          style: ElevatedButton.styleFrom(
                                            backgroundColor: Colors.white38,
                                            alignment:
                        ...
                        return Container(
                          ...
                          child: GestureDetector(
                              ...
                              Navigator.push(
                                ...
                                PageRouteBuilder(
                                  pageBuilder: (context, animation,
                                      secondaryAnimation) =>
                                      MovieScreen(
                                        slug: newMoviesList[index].slug.toString(),
                                      ),
                                  transitionsBuilder: (context, animation,
                                      secondaryAnimation, child) {
                                    const begin = Offset(1.0, 0.0);
                            ...
                            child: Container(
                              ...
                              child: CachedNetworkImage(
                                imageUrl:
                                newMoviesList[index].posterUrl.toString(),
                                placeholder: (context, url) =>
                                    Shimmer.fromColors(
                                      baseColor: Colors.grey,
                                      highlightColor: Colors.white,
            ...
            SizedBox(
              height: 200,
              child: FutureBuilder(
                future: _futureSingleMovies,
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.done) {
                    return ListView.builder(
                      scrollDirection: Axis.horizontal,
                      itemCount: singleMoviesList.length,
                      itemBuilder: (context, index) {
                        return Container(
                          margin: const EdgeInsets.only(left: 10),
                          // height: 100,
                          ...
                          child: GestureDetector(
                              ...
                              Navigator.push(
                                ...
                                PageRouteBuilder(
                                  pageBuilder: (context, animation,
                                      ...
                                      MovieScreen(
                                        slug:
                                        singleMoviesList[index].slug.toString(),
                                      ),
                                  transitionsBuilder: (context, animation,
                                      secondaryAnimation, child) {
                                    const begin =
                            ...
                            child: Container(
                              ...
                              child: CachedNetworkImage(
                                imageUrl:
                                'https://kkphim.com/${singleMoviesList[index].posterUrl}',
                                placeholder: (context, url) =>
                                    Shimmer.fromColors(
                                      baseColor: Colors.grey,
                                      highlightColor: Colors.white,
            ...
            SizedBox(
              height: 200,
              child: FutureBuilder(
                future: _futureSeriesMovies,
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.done) {
                    return ListView.builder(
                      scrollDirection: Axis.horizontal,
                      itemCount: seriesMoviesList.length,
                      itemBuilder: (context, index) {
                        return Container(
                          margin: const EdgeInsets.only(left: 10),
                          // height: 100,
                          ...
                          child: GestureDetector(
                              ...
                              Navigator.push(
                                ...
                                PageRouteBuilder(
                                  pageBuilder: (context, animation,
                                      ...
                                      MovieScreen(
                                        slug:
                                        seriesMoviesList[index].slug.toString(),
                                      ),
                                  transitionsBuilder: (context, animation,
                                      secondaryAnimation, child) {
                                    const begin =
                            ...
                            child: Container(
                              ...
                              child: CachedNetworkImage(
                                imageUrl:
                                'https://www.kkphim.vip/${seriesMoviesList[index].posterUrl}',
                                placeholder: (context, url) =>
                                    Shimmer.fromColors(
                                      baseColor: Colors.grey,
                                      highlightColor: Colors.white,
            ...
            SizedBox(
              height: 200,
              child: FutureBuilder(
                future: _futureCartoonMovies,
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.done) {
                    return ListView.builder(
                      scrollDirection: Axis.horizontal,
                      itemCount: cartoonMoviesList.length,
                      itemBuilder: (context, index) {
                        return Container(
                          margin: const EdgeInsets.only(left: 10),
                          // height: 100,
                          ...
                          child: GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: CachedNetworkImage(
                                imageUrl:
                                'https://www.kkphim.vip/${cartoonMoviesList[index].posterUrl}',
                                placeholder: (context, url) =>
                                    Shimmer.fromColors(
                                      baseColor: Colors.grey,
                                      highlightColor: Colors.white,
                        ...
                        return Container(
                          ...
                          child: GestureDetector(
                              ...
                              Navigator.push(
                                ...
                                PageRouteBuilder(
                                  pageBuilder: (context, animation,
                                      secondaryAnimation) =>
                                      MovieScreen(
                                        slug: tvShowsList[index].slug.toString(),
                                      ),
                                  transitionsBuilder: (context, animation,
                                      secondaryAnimation, child) {
                                    const begin =
                            ...
                            child: Container(
                              ...
                              child: CachedNetworkImage(
                                imageUrl:
                                'https://www.kkphim.vip/${tvShowsList[index].posterUrl}',
                                placeholder: (context, url) =>
                                    Shimmer.fromColors(
                                      baseColor: Colors.grey,
                                      highlightColor: Colors.white,
